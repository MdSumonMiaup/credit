  ...
  class CSF {
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( apply_filters( 'csf_fa4', false ) ) {
		...
		wp_enqueue_style( 'csf-custom-font', self::include_plugin_url( 'assets/css/credit-icon.css' ), array(), self::$version, 'all' );
      }
      ...
      wp_localize_script( 'csf', 'csf_vars', array(
        ...
        'i18n'              => array(
          'confirm'         => esc_html__( 'Are you sure?', 'credit' ),
          'typing_text'     => esc_html__( 'Please enter %s or more characters', 'credit' ),
          'searching_text'  => esc_html__( 'Searching...', 'credit' ),
          'no_results_text' => esc_html__( 'No results found.', 'credit' ),
        ),
      ) );
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field['_notice'] ) ) {
        ...
        $field            = array();
        $field['content'] = esc_html__( 'Oops! Not allowed.', 'credit' ) .' <strong>('. $field_type .')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      ...
      if ( ! empty( $field_type ) ) {
        ...
        if ( class_exists( $classname ) ) {
          ...
          echo '<p>'. esc_html__( 'Field not found!', 'credit' ) .'</p>';
        }
